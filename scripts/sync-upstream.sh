...
# To update: change PINNED_TAG to the desired tag (e.g. b9300)
...
for wf in "${CUSTOM_WORKFLOWS[@]}"; do
    if [ -f "$REPO/.github/workflows/$wf" ]; then
        cp "$REPO/.github/workflows/$wf" "$BACKUP/workflows/$wf"
        success "Backed up: .github/workflows/$wf"
    else
        warn "Workflow not found, skipping: $wf"
    fi
done

# ── Fetch Upstream ────────────────────────────────────────────
info "Fetching upstream (tag: ${PINNED_TAG})..."
git fetch upstream --tags --force || die "Failed to fetch upstream"
...
info "Removing upstream workflows..."
if [ -d "$REPO/.github/workflows" ]; then
    rm -f "$REPO/.github/workflows"/*.yml
    success "Removed upstream workflows"
fi
...
# Restore script to $SELF as well
...
for wf in "${CUSTOM_WORKFLOWS[@]}"; do
    if [ -f "$BACKUP/workflows/$wf" ]; then
        cp "$BACKUP/workflows/$wf" "$REPO/.github/workflows/$wf"
        success "Restored: .github/workflows/$wf"
    else
        warn "Workflow backup not found: $wf"
    fi
done
...
echo ""
echo -e "  ${YELLOW}Windows clone?${RESET} Run on your Windows machine:"
echo -e "  ${CYAN}git fetch origin && git reset --hard origin/master${RESET}"
echo ""
